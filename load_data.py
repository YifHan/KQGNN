...
class SocialDataset(Dataset):
    def __init__(self, path, index):
        ...
        self.img_features = np.load(path + '/' + str(index) + '/im_features.npy')
    ...
    def remove_obsolete_nodes(self, indices_to_remove=None):  
        if indices_to_remove is not None:
            ...
            self.img_features = self.img_features[indices_to_keep, :]
...
def getdata(embedding_save_path,data_split,i,args):
    ...
    img_features = torch.FloatTensor(data.img_features)
    ...
    if args.use_cuda:
        g = g.to(device)
        features, labels , img_features= features.cuda(), labels.cuda(), img_features.cuda()
        test_indices = test_indices.cuda()
        if i==0:
            train_indices, validation_indices = train_indices.cuda(), validation_indices.cuda()

    g.ndata['features'] = features
    g.ndata['img_features'] = img_features
    g.ndata['labels'] = labels


    if i==0:
        return save_path_i, img_dim, in_feats, num_isolated_nodes, g, labels, train_indices, validation_indices, test_indices
    else:
        return save_path_i, img_dim, in_feats, num_isolated_nodes, g, labels, test_indices
